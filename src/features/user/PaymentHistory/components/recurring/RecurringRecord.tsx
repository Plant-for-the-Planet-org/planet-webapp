...
export const RecurringRecord = ({ record, onAction }: RecurringRecordProps) => {
  ...
  // Labels via literal keys (dynamic t(status)/t(method) would break the typed
  // messages), matching the legacy card's values.
  const frequencyLabel = record.frequency === 'yearly' ? t('yearly') : t('monthly');
  ...
  return (
    <div className="rounded-xl border border-solid border-border bg-card p-5">
      ...
      <dl className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-3">
        ...
        {dest?.type === 'mixed' ? (
          <Detail
            label={t('projects')}
            value={dest.items.map((item) => item.name).join(', ')}
          />
        ) : dest?.type === 'planet-cash' ? (
