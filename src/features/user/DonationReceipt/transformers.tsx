...
import type {
    ...
    DonorView,
    AddressView,
    IssuedDonationApi, DonationView, UnissuedDonationApi
} from "./donationReceiptTypes";
import {Address} from "@planet-sdk/common";
...
export const transformDonor = (
    ...
    tin: donor.tin ?? null,
    type: donor.type === 'individual' || donor.type === 'organization'
        ? donor.type
        : null,
});
...
export const transformProfileToDonorView = (user: User): DonorView => ({
    name: user.type === 'individual'
        ...
        : user.name ?? null,
    tin: user.tin ?? null,
    type: user.type === 'individual' || user.type === 'organization'
        ? user.type
        : null,
})
...
export const transformProfileToPrimaryAddressView = (user: User): AddressView | null => {
...
export const transformProfileToPrimaryAddressGuid = (user: User): string | null => {
    const primaryAddress = getPrimaryAddress(user);
