    ...
    <>
      ...
      >
        ...
        <Layer
          ...
          filter={['!=', ['get', 'dateDiff'], '']}
        />
        {shouldShowSampleMarkers ? selectedPlantLocation.sampleInterventions.map((spl) => (
          <Marker
            key={`${spl.id}-sample`}
          ...
          >
            <div
              key={`${spl.id}-marker`}
              className={`${styles.single} ${spl.hid === selectedSamplePlantLocation?.hid ? styles.singleSelected : ''
                }`}
              role="button"
              tabIndex={0}
