...
| **Status** | 🟡 Remediated in code (2026-07-28) — pending verification |

**Remediation:** All eleven modals now carry `role="dialog"` with `useId()`-generated `aria-labelledby` (and `aria-describedby` where a description exists) **on the modal's content element**. The inert placeholder attributes were removed from the `<Modal>` wrappers: MUI's `Modal` root renders `role="presentation"`, so ARIA naming attributes passed to `<Modal>` never applied, valid IDREF or not. `RedeemModal` uses `aria-label={t('redeem')}` instead, because its visible heading changes with the redeem state. Escape dismissal was added to `CustomModal` (`onClose={handleCancel}`), `EmbedModal` (new shared `handleCancel`, which is also what its cancel button now calls), `ImageSliderModal` (escape only, so backdrop clicks keep doing nothing) and the mobile `MapFeatureExplorer` panel (previously it filtered for `backdropClick` and swallowed Escape). `EmbedModal`'s duplicate `id="editProfileSaveProfile"` on the cancel button became `id="editProfileCancel"`. Focus management is MUI's and was **not** re-implemented — but one modal silently disabled it, because `FocusTrap` clones the `Modal` child with a ref: `ImageSliderModal` returned a fragment (now a single `<div>` carrying the dialog role and the flex layout the modal root used to apply — including a 20px side padding that reproduces the edge inset MUI's focus-trap sentinels used to create as extra `gap`; without it the 800px image container stops shrinking to the viewport and the mobile arrows and close button fall off-screen). `LayerInfoPopup` was intentionally left unchanged — see below. No visual or behavioural change. Line numbers below reflect the pre-fix code.

**Not fixed here:** `LayerInfoPopup.tsx` is listed below but is not a MUI modal — it is a plain conditionally-rendered popup. Giving it an accessible name only helps once it has dialog semantics and focus management, which is exactly the scope of **A11Y-023**; splitting that work across two findings would leave it half-done. Its missing-Escape entry is a false positive: the parent `SiteMapLayerControls/index.tsx:29`–`33` already closes it on Escape.
...
- **Status:** The eleven modals listed under A11Y-011 are remediated in place (2026-07-28); LayerInfoPopup is tracked under A11Y-023. Five further modals with the same defect fall outside A11Y-011's file list and are tracked as **FU-006**; the shared `<AppModal>` primitive is tracked as **FU-007** (`FOLLOW_UPS.md`).
